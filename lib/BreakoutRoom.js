...
export default class BreakoutRoom {
  ...
  constructor(callFrame, joinCall, room, createToken, eventHandlers = null) {
    ...
    if (eventHandlers) {
      ...
      this.onBreakoutSync = eventHandlers?.onBreakoutSync;
    }
  ...
  leaveSession = () => {
    ...
    if (roomIdx > -1) {
      ...
      r[roomIdx].participants = r[roomIdx].participants.filter(
        p => p.user_id !== localParticipant.user_id,
      );
      this.breakoutSession.rooms = r;
      this._sendToSocket('DAILY_BREAKOUT_UPDATED', this.breakoutSession);
    }
    this._syncSession(this.breakoutSession);
  ...
  assignRoomToNewParticipant = (participant, roomName = null) => {
    ...
    if (roomName) {
      ...
      if (participantOldRoomIndex > -1) {
        ...
        const oldParticipantIds = oldParticipants.map(p => p.user_id);
        oldRoom = {
  ...
  onBreakoutSessionUpdated = async data => {
    ...
    const myBreakoutRoom = this.getMyBreakoutRoom();
    if (myBreakoutRoom?.roomName && myBreakoutRoom?.roomName !== room?.name) {
      await this._join(this.myBreakoutRoom.roomName, true);
    }
  ...
  onBreakoutSessionConcluded = data => {
    ...
    if (this.currentRoom !== this.room) this._join(this.room, false);
  ...
  onBreakoutSessionSync = async data => {
    this._syncSession(data.sessionObject);
    if (this.onBreakoutSync) this.onBreakoutSync(this);
  };

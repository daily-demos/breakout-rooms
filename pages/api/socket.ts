  ...
  if (req.method === 'POST') {
    if (event === 'DAILY_BREAKOUT_STARTED') {
      ...
      sessionObject.rooms.map(async (room: DailyBreakoutRoom) => {
        ...
        const alreadyExists = await checkIfRoomExists(room.roomName);
        if (alreadyExists) {
          retCode = await updateRoomProperties(room.roomName, roomProperties, {
            enable_recording: 'cloud',
          });
        } else {
          retCode = await createRoom(room.roomName, roomProperties, {
            enable_recording: 'cloud',
          });
...
// trying to check if the room already exists.
function checkIfRoomExists(roomName: string) {
  const options = {
    method: 'GET',
    headers: {
...
function updateRoomProperties(
  ...
  return fetch(url, options).then(async response => {
    ...
    console.log(
      `Failed to update breakout room. Status code: ${response.status}, error: ${data?.info}`,
    );
